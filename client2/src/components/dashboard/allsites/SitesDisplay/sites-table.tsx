...
export default function SitesTable({
  ...
  const onSuccess = (res: AxiosResponse<SuccessResponse>): void => {
    ...
    if(reset){
      setReset(false);
      setSite(res.data.sites.find((site) => site._id === siteId) || res.data.sites[0]);
    }
  };
  ...
  React.useEffect(() => {
    ...
    if (filters.isSearching && searchResults.length > 0) {
      ...
      setSites(
        searchResults.slice((pagination.page - 1) * config.SitesPageSize, pagination.page * config.SitesPageSize)
      );
    }
  ...
  React.useEffect(() => {
    if (reset) {
      loadPage();
      fetchSites({ page: pagination.page, filters: { isRunning: false, isStopped: false, isSearching: false } });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps -- reset is the only dependency
  }, [reset]);
                ...
                <TableRow sx={{ cursor: 'pointer' }} hover key={site._id} onClick={() => handleRowClick(site)}>
                  ...
                  <TableCell>
                    <Typography
                      variant="body1"
                      fontWeight={600}
                      color={site.Status === 0 ? 'var(--mui-palette-error-dark)' : 'var(--mui-palette-primary-dark)'}
                    >
